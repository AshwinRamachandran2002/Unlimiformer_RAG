            ...
            for i in range(0, len(lines), 7):
                ...
                if '[' not in seg[0]:
                    ...
                    if numbers_tokens[0] >= start and numbers_tokens[0] <= end:
                        pass
                    elif numbers[0] >= 0.01 and head in selected_heads_per_layer[layer_number]:
                        # print("FLAG!!")
                        # print("layer", layer_number)
                        # print("head", head, "predicting", numbers_tokens[0], "score", numbers[0])
                        if head not in flags:
                            flags[head] = [(j, seq, numbers_tokens[0], text[numbers_tokens[0] - 1], text[numbers_tokens[0]], text[numbers_tokens[0] + 1], text[numbers_tokens[0] + 2], text[numbers_tokens[0] + 3])]
                        else:
                            flags[head].append((j, seq, numbers_tokens[0], text[numbers_tokens[0] - 1], text[numbers_tokens[0]], text[numbers_tokens[0] + 1], text[numbers_tokens[0] + 2], text[numbers_tokens[0] + 3]))
