        ...
        with torch.no_grad():
            ...
            if self.use_datastore:
                ...
                if self.reconstruct_embeddings:
                    ...
                    top_search_key_indices = torch.cat((torch.sort(top_search_key_indices).values, torch.arange(self.update_end - self.cover_end, self.update_end).unsqueeze(0).unsqueeze(0).repeat(1, 40, 1)), dim=2)
                    send_indices = torch.cat((torch.sort(send_indices).values, self.chunk_position_ids[0][torch.arange(self.update_end - self.cover_end, self.update_end)].unsqueeze(0).unsqueeze(0).repeat(1, 40, 1)), dim=2)     
                    for head in range(40):
