...
class Unlimiformer(Generic[ModelType]):
            ...
            if self.csv_unlimiformer:
                ...
                if self.ind_up >= 2:
                    ...
                    if self.ind_up == self.num_data:
                        chunk_position_ids = torch.cat((torch.arange(0, (self.ind_up) * self.data_len)[0 : self.data_len * self.num_anchors], torch.arange(0, (self.ind_up) * self.data_len - 1)[ -((self.num_templates + 1)* self.data_len - 1) :])).unsqueeze(0).to(self.device)
                    elif self.ind_up >= self.num_anchors + self.num_templates + 1:
                        chunk_position_ids = torch.cat((torch.arange(0, (self.ind_up) * self.data_len)[0 : self.data_len * self.num_anchors], torch.arange(0, (self.ind_up) * self.data_len)[ -((self.num_templates + 1)* self.data_len) :])).unsqueeze(0).to(self.device)
                    else:
                    ...
                    for i, layer_states in enumerate(to_add_embeddings):
                        ...
                        if self.csv_unlimiformer:
                            if self.ind_up == 1:
                                ...
                                self.hidden_layer_our[i] = torch.cat((self.hidden_layer_our[i][:, : self.data_len * self.num_anchors], self.hidden_layer_our[i][:, -(self.data_len * (self.num_templates - 1)):], layer_states), dim = -2).to(self.datastore_device)
                if self.csv_unlimiformer:
    ...
    def pre_generate_hook(self, input_ids, **kwargs):
        ...
        if self.is_encoder_decoder:
            ...
            if self.csv_unlimiformer:
                vals_pred = []
                for i in range(2 * self.num_data):
                    self.curr_key = i
